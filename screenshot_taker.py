...
class MousePositionTracker(tk.Frame):
    ...
    def begin(self, event):
        global x1; global y1
        x1 = event.x + self.canvas.winfo_rootx() # add coordinate of tkinter window to find position on screen
        y1 = event.y + self.canvas.winfo_rooty()
        self.hide()
    ...
    def quit(self, event):
        global x2; global y2
        x2 = event.x + self.canvas.winfo_rootx()
        y2 = event.y + self.canvas.winfo_rooty()
        print("final coordinates: ", x2, " ", y2)
        screenshot_of_region(self,event)
...
def screenshot_of_region(self,event):
    ...
    if (x2<=x1): # if you drag window bottom right to top left
